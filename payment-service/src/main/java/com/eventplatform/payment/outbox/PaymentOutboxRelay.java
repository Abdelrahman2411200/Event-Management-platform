...
public class PaymentOutboxRelay {
    private static final Logger LOGGER = LoggerFactory.getLogger(PaymentOutboxRelay.class);
    ...
    public PaymentOutboxRelay(
            ...
            @Value("${platform.outbox.topic:event-platform.payment-lifecycle.v1}") String topic,
        ...
        this.clock = clock;
        this.published = meterRegistry.counter("platform.outbox.publications", "service", "payment-service", "result", "published");
        this.failed = meterRegistry.counter("platform.outbox.publications", "service", "payment-service", "result", "retry");
        this.deadLettered = meterRegistry.counter("platform.outbox.publications", "service", "payment-service", "result", "dead-lettered");
    }

    @Scheduled(fixedDelayString = "${platform.outbox.publish-interval:1s}")
    @Transactional
    public void publishPending() {
        for (PaymentOutboxMessage message : repository.findPending(
            ...
            try {
                ...
                KafkaEventHeaders.write(record.headers(), new KafkaEventMetadata(
                        ...
                        "payment-service", message.getAggregateType(), message.getAggregateId().toString()));
                ...
                if (message.isDeadLettered()) {
                    ...
                    LOGGER.error("Payment outbox message dead-lettered messageId={} eventType={} aggregateId={} attempts={}",
                            message.getId(), message.getEventType(), message.getAggregateId(), message.getPublishAttempts());
                } else {
                    failed.increment();
                    LOGGER.warn("Payment outbox publication scheduled for retry messageId={} eventType={} attempts={}",
