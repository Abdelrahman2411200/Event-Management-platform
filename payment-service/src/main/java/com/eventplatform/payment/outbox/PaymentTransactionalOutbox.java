...
public class PaymentTransactionalOutbox {
    ...
    public void append(String aggregateType,UUID aggregateId,String eventType,Object payload,RequestContext context,Instant at){try{repository.save(new PaymentOutboxMessage(UUID.randomUUID(),aggregateType,aggregateId,eventType,mapper.writeValueAsString(payload),context.correlationId(),context.traceparent(),at));}catch(JsonProcessingException e){throw new PaymentApiException(HttpStatus.INTERNAL_SERVER_ERROR,"OUTBOX_SERIALIZATION_FAILED","The payment state change could not be recorded");}}
}
