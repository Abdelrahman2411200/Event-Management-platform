...
public class PaymentStateService {
 ...
 private void append(Payment p,String type,RequestContext context,Instant now){outbox.append("Payment",p.getId(),type,new PaymentLifecycleEvents.PaymentChangedV1(p.getId(),p.getBookingId(),p.getAttendeeId(),p.getEventId(),p.getAmount(),p.getCurrency(),p.getStatus(),p.getFailureCode(),p.getFailureReason(),now),context,now);}
