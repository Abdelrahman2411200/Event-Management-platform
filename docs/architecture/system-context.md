...
Phase 1 establishes a local-first platform foundation. It contains buildable service processes, local infrastructure, migrations, health and API documentation endpoints, observability wiring, and a frontend shell. It intentionally contains no authentication, event, venue, attendee, booking, payment, or notification business behavior.
...
| --- | --- | --- |
| `api-gateway` | Public routing, edge correlation, future edge authentication and rate policies | Domain rules or persistent domain data |
| `auth-service` | Future identities, credentials, tokens, roles, and authorization policy | Attendees, events, or payments |
...
The outbox schema is intentionally absent from Phase 1 because there are no domain events yet. It will be added by the first service that publishes one, with a reusable operational pattern but service-owned migrations.
...
No object-storage flow exists in Phase 1, so MinIO is not started. When an object use case is introduced, the first adapter will support a local S3-compatible provider and a cloud provider through the same port without making AWS credentials a local prerequisite.
